...
if (new Date - user.lastweekly < 259200000) return await conn.sendButton(m.chat, ` لقد تلقيت بالفعل مكافأتك الأسبوعية ⛅`, wm + `\n عد في\n${clockString(time - new Date() * 1)}`, null, [['اك', 'a']], fkontak, m)
let texto = ''
...
await conn.sendButton(m.chat, text, texto + `\n\n🎟️ 𝗣 𝗥 𝗘 𝗠 𝗜 𝗨 𝗠 ⇢ ${premium ? '✅' : '❌'}\n${wm}`, gata.getRandom(), [['🌅   شهر  🌠', '/شهريه'], [' يوميه ☘️', 'يومي']], m, enlace)  
user.lastweekly = new Date * 1
}
handler.command = ['weekly', 'semana', 'اسبوعي', 'اسبوعيه', 'entregasemanal'] 
handler.level = 7
