...
if (!text) return conn.reply(m.chat, `${mg}*أدخل عدد الوقت المميز*\n*✤ 🎟️ 1 = ${tiempoDecretado} دقيقة*\n*✤ ${gata} ${rpgshop.emoticon('limit')}*\n\n*مثال: ${usedPrefix + command} 1*`, fkontak, m)
if (isNaN(text)) return conn.reply(m.chat, `${mg} استخدم الأرقام\n\n*مثال: ${usedPrefix + command} 1*`, fkontak, m)
if (user.limit < gata) return conn.reply(m.chat, `${ag} ليس لديك ما يكفي *${rpgshop.emoticon('limit')}*  من أجل حصول عليه🎟️ بريميوم العربة ${rpgshopp.emoticon('limit')} في المتجر باستخدام *${usedPrefix}شراء*أو يمكنك البيع لتحقيق ربح باستخدام الأمر*${usedPrefix}بيع*`, fkontak, m)
...
user.premium = true
const imgpre = [ 
'https://telegra.ph/file/3885dd871371251b4931e.jpg', 
'https://telegra.ph/file/3885dd871371251b4931e.jpg ',
'https://telegra.ph/file/a9ba2792a63d7b389dc1f.jpg '];
//let imgpre = 'https://logowiki.net/wp-content/uploads/imgp/Premium-Logo-1-5365.jpg' 

await conn.sendButton(m.chat,`${eg}*╭┈┈┈┈┈◈ 🌟 ◈┈┈┈┈┈╮*
*┃🎟️ تم شراء*
*┃✨ الاسم*  ${user.name}*
*┃💰 دفع »  -${gata * text} ${rpgshopp.emoticon('limit')}*
*┃👝 كان » ${user.limit + gata} ${rpgshopp.emoticon('limit')}*
*┃🛄 الباقي  » ${user.limit} ${rpgshopp.emoticon('limit')}*
*┃🕐 وقت » ${tiempoPremium} دقيقه*
*┃📉 الموقت » ${user.premiumTime - now} ثانيه*
*╰┈┈┈┈┈◈ 🌟 ◈┈┈┈┈┈╯*`, `*الآن لديك بريميوم لذلك لن يكون هناك حدود.*\n\n${wm}`, imgpre[Math.floor(Math.random() * imgpre.length)], [
[`🎟️ شراء المزيد من الوقت🎟️`, `${usedPrefix + command} 1`],
[`الاوامر`, `${usedPrefix}menu`]], fkontak, m)}
...
[`😽 𝗗𝗜𝗦𝗙𝗥𝗨𝗧𝗔𝗥 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 😽`, `${usedPrefix}menu`]], fkontak, m)}
...
if (!text) return conn.reply(m.chat, `${mg}*𝙄𝙉𝙂𝙍𝙀𝙎𝙀 𝙀𝙇 𝙉𝙐𝙈𝙀𝙍𝙊 𝘿𝙀 𝙏𝙄𝙀𝙈𝙋𝙊 𝙋𝙍??𝙈𝙄𝙐𝙈*\n𝙀𝙉𝙏𝙀𝙍 𝙏𝙃𝙀 𝙋𝙍𝙀𝙈𝙄𝙐𝙈 𝙏𝙄𝙈𝙀 𝙉𝙐𝙈𝘽𝙀𝙍\n\n*✤ 🎟️ 1 = ${tiempoDecretado} HORA(S)*\n*✤ ${gata} ${rpgshop.emoticon('diamond')}*\n\n*EJEMPLO: ${usedPrefix + command} 1*`, fkontak, m)
if (isNaN(text)) return conn.reply(m.chat, `${mg}𝙎𝙊𝙇𝙊 𝙎𝙀 𝘼𝘾𝙀𝙋𝙏𝘼 𝙉𝙐𝙈𝙀𝙍𝙊𝙎\n𝙊𝙉𝙇𝙔 𝙉𝙐𝙈𝘽𝙀𝙍𝙎 𝘼𝘾𝘾𝙀𝙋𝙏𝙀𝘿\n\n*EJEMPLO: ${usedPrefix + command} 1*`, fkontak, m)
...
{title: "🚀 𝗗 𝗜 𝗦 𝗙 𝗥 𝗨 𝗧 𝗔 𝗥   𝗣 𝗥𝗘 𝗠 𝗜 𝗨 𝗠", rowId: usedPrefix + 'menu'}
