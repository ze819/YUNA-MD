...
if (new Date - user.lastmiming < 10000) return await conn.reply(m.chat, `*انتظر بضع دقائق لاستخدام أمر *`,  m)
  try {
  	
    let q = m.quoted ? m.quoted : m
    let mime = (q.msg || q).mimetype || q.mediaType || ''
    if (/webp|image|video/g.test(mime)) {
      if (/video/g.test(mime)) if ((q.msg || q).seconds > 11) return m.reply('╰⊱⚠️⊱ *تحذير* ⊱⚠️⊱╮\nيجب ألا يدوم الفيديو أكثر من * 7 * ثوان')
      ...
      await conn.reply(m.chat, `${eg}⏳ *ثواني و يوصلك الملصق* `, m)
        if (!stiker) {
