...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "YunaJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])))
}, 30 * 1000)}
...
function purgeSession() {
    let prekey = []
    let directorio = readdirSync("./YunaBotSession")
    let filesFolderPreKeys = directorio.filter(file => {
        return file.startsWith('pre-key-')
    })
    prekey = [...prekey, ...filesFolderPreKeys]
    filesFolderPreKeys.forEach(files => {
    unlinkSync(`./YunaBotSession/${files}`)
})

}  
function purgeSessionSB() {
let listaDirectorios = readdirSync('./YunaJadiBot/');
console.log(listaDirectorios)
      let SBprekey = []
listaDirectorios.forEach(filesInDir => {
    let directorio = readdirSync(`./YunaJadiBot/${filesInDir}`)
    ...
    DSBPreKeys.forEach(fileInDir => {
        unlinkSync(`./YunaJadiBot/${filesInDir}/${fileInDir}`) 
    })
    })
...
const directories = ['./YunaBotSession/', './YunaJadiBot/']
