...
export async function handler(chatUpdate) {
    ...
    try {
		...
		if (!('registroC' in user)) user.registroC = false    
		    ...
		    if (!isNumber(user.identidad)) user.identidad = -1
		    if (!isNumber(user.pasatiempo)) user.pasatiempo = -1
...
this.sendButton(id, wm, text, apii.data, [[(action == 'add' ? lenguajeGB.smsCont13() : lenguajeGB.smsCont14()), (action == 'add' ? '.s' : '.s')], ['', ``]], null, {mentions: this.parseMention(text)})
}}}
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, 'بوت 🧸 ' + gt + ' 👑', '🌟 ahmedelbakour@iissdontoninotricase.net '].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, yt, ig, paypal, fb].getRandom() }}}}, tg)
